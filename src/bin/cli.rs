...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Analyze { profile } => lwoodz::use_case::run(&profile, cli.json),
        ...
        Commands::ServiceTerms => cmd_service_terms(cli.json),
        Commands::Openness => cmd_openness(cli.json),
...
fn cmd_service_terms(json: bool) -> anyhow::Result<()> {
    let cfg = load_cfg()?;
    lwoodz::service_terms::run(&cfg.repo_path, json)
}

fn cmd_openness(json: bool) -> anyhow::Result<()> {
    let cfg = load_cfg()?;
    lwoodz::openness::run(&cfg.repo_path, json)
}
