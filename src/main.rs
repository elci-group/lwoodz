...
async fn main() -> anyhow::Result<()> {
    if let Err(e) = lwoodz::util::dotenv::load() {
        eprintln!("lwoodz: dotenv load failed: {e}");
    }

    let cli = Cli::parse();
    if let Some(profile) = &cli.analyze {
        return lwoodz::use_case::run(profile, cli.json);
    }
    ...
    if cli.service_terms {
        return lwoodz::service_terms::run(&config.repo_path, cli.json);
    }

    if cli.openness {
        return lwoodz::openness::run(&config.repo_path, cli.json);
    }
